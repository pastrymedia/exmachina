/**
 * This file controls the behaviours on the design settings page.
 *
 * @since 0.9.7
 * @version 1.0
 */
...
jQuery(document).ready(function($) {
	var isUnsaved = false, $design_settings = $('.wrap');

	// Start with all post boxes closed
	if (exmachina.firstTime) {
		$('div.postbox').addClass('closed');
		postboxes.save_state(exmachina.pageHook);
	}
	...
	$('<button id="toggle-meta-boxes" class="button button-secondary">' + exmachina.toggleAll + '</button>"')
	...
	$('.button-reset').click(function() {
		return exmachina_confirm(exmachina.warnReset);
	})
