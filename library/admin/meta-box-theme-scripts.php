...
add_action( 'add_help_tabs', 'hybrid_theme_settings_scripts_help');
...
?>
	<p>
		<label for="<?php echo hybrid_settings_field_id( 'header_scripts' ); ?>"><?php printf( __( 'Insert scripts or code before the closing %s tag in the document source', 'hybrid-core' ), '<code>&lt;/head&gt;</code>' ); ?>:</label>
	</p>
	...
	<p>
		<label for="<?php echo hybrid_settings_field_id( 'footer_scripts' ); ?>"><?php printf( __( 'Insert scripts or code before the closing %s tag in the document source', 'hybrid-core' ), '<code>&lt;/body&gt;</code>' ); ?>:</label>
	</p>
 ...
 */
function hybrid_theme_settings_scripts_help() {

	$screen = get_current_screen();

	$scripts_help =
		'<h3>' . __( 'Header and Footer Scripts', 'hybrid-core' ) . '</h3>' .
		'<p>'  . __( 'This provides you with two fields that will output to the head section of your site and just before the closing body tag. These will appear on every page of the site and are a great way to add analytic code, Google Font and other scripts. You cannot use PHP in these fields.', 'hybrid-core' ) . '</p>';

	$screen->add_help_tab( array(
		'id'      => hybrid_get_settings_page_name() . '-scripts',
		'title'   => __( 'Header and Footer Scripts', 'hybrid-core' ),
