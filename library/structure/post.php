<?php
/**
 * Hybrid Framework
 *
 * WARNING: This file is part of the core Hybrid Framework DO NOT edit this file under any circumstances.
 * Please do all modifications in the form of a child theme.
 *
 * @package Hybrid\Entry
 * @author  Machina Themes
 * @license GPL-2.0+
 * @link    http://machinathemes.com
 */


/* Add the title, byline, and entry meta before and after the entry.*/
add_action( hybrid_get_prefix() . '_before_entry', 'hybrid_entry_header' );
add_action( hybrid_get_prefix() . '_entry', 'hybrid_entry' );
add_action( hybrid_get_prefix() . '_singular_entry', 'hybrid_singular_entry' );
add_action( hybrid_get_prefix() . '_after_entry', 'hybrid_entry_footer' );
add_action( hybrid_get_prefix() . '_singular-page_after_entry', 'hybrid_page_entry_meta' );

add_filter('excerpt_more', 'hybrid_excerpt_more');

/**
 * Display the default entry header.
 */
function hybrid_entry_header() {

  echo '<header class="entry-header">';

  if ( is_home() || is_archive() || is_search() ) {
  ?>
    <h1 class="entry-title" itemprop="headline"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
  <?php
  } else {
  ?>
    <h1 class="entry-title" itemprop="headline"><?php the_title(); ?></h1>
  <?php
  }
  if ( 'post' == get_post_type() ) :
    get_template_part( 'partials/entry', 'byline' );
  endif;
  echo '</header><!-- .entry-header -->';

}

/**
 * Display the default entry metadata.
 */
function hybrid_entry() {

  if ( is_home() || is_archive() || is_search() ) {
    if(hybrid_get_setting( 'content_archive_thumbnail' )) {
      get_the_image( array( 'meta_key' => 'Thumbnail', 'default_size' => hybrid_get_setting( 'image_size' ) ) );
    }


    if ( 'excerpts' === hybrid_get_setting( 'content_archive' ) ) {
      if ( hybrid_get_setting( 'content_archive_limit' ) )
        the_content_limit( (int) hybrid_get_setting( 'content_archive_limit' ), hybrid_get_setting( 'content_archive_more' ) );
      else
        the_excerpt();
    }
    else {
      the_content( hybrid_get_setting( 'content_archive_more' ) );
    }
  }

}

/**
 * Display the default singular entry metadata.
 */
function hybrid_singular_entry() {

  the_content();

  wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Pages:', 'hybrid-core' ) . '</span>', 'after' => '</p>' ) );

}

/**
 * Display the default entry footer.
 */
function hybrid_entry_footer() {

  if ( 'post' == get_post_type() ) {
    get_template_part( 'partials/entry', 'footer' );
  }

}

/**
 * Display the default page edit link
 */
function hybrid_page_entry_meta() {

  echo apply_atomic_shortcode( 'entry_meta', '<div class="entry-meta">[entry-edit-link]</div>' );
}


function hybrid_excerpt_more( $more ) {
  return ' ... <a class="more-link" href="'. get_permalink( get_the_ID() ) . '">' . hybrid_get_setting( 'content_archive_more' ) . '</a>';
}

