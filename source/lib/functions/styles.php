...
function exmachina_get_styles() {
  ...
  if ( is_child_theme() ) {
    ...
    $src = trailingslashit( PARENT_THEME_URL ) . "style.css";

    /* If a '.min' version of the parent theme stylesheet exists, use it. */
    if ( !empty( $suffix ) && file_exists( trailingslashit( PARENT_THEME_DIR ) . "style{$suffix}.css" ) )
      $src = trailingslashit( PARENT_THEME_URL ) . "style{$suffix}.css";
