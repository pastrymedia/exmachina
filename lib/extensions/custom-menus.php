<?php
/*
Plugin Name: ExMachina Simple Menus
Description: ExMachina Simple Menus allows you to select a WordPress menu for secondary navigation on individual posts/pages.
Version: 0.2
*/

/* Sample implementation of adding support for custom taxonomy

add_filter( 'exmachina_simple_menus_taxonomies', 'gsm_sample_taxonomy' );
function gsm_sample_taxonomy( $taxonomies ) {
	$taxonomies[] = 'taxonomy-slug';
	return array_unique( $taxonomies );
}
*/
class ExMachina_Simple_Menus {
 ...
 */
	function init() {
		...
		$this->taxonomies = apply_filters( 'exmachina_simple_menus_taxonomies', array_keys( $_taxonomies ) );
 ...
 */
	function admin_menu() {
		...
		foreach( (array) get_post_types( array( 'public' => true ) ) as $type ) {
			...
			if( $type == 'post' || $type == 'page' || post_type_supports( $type, 'exmachina-simple-menus' ) )
 ...
 */
	...
	<p>
		<?php
		$this->print_menu_select( $this->field_name, exmachina_get_custom_field( $this->field_name ), 'width: 99%;' );
		?>
	</p>
	<?php
	}
 ...
 *  giddyup
 */
$gsm_simple_menu = new ExMachina_Simple_Menus();
