/**
 * This file controls the behaviours within the ExMachina Framework.
 *
...
/*global exmachina_confirm, confirm, jQuery, exmachina, exmachina_toggles, exmachinaL10n */

/**
 * Holds ExMachina values in an object to avoid polluting global namespace.
 *
 * @since 1.8.0
 *
 * @constructor
...
window['exmachina'] = {
	...
	category_checklist_toggle_init: function () {
		...
		jQuery('<p><span id="exmachina-category-checklist-toggle" class="button">' + exmachinaL10n['categoryChecklistToggle'] + '</span></p>').insertBefore('ul.categorychecklist');

		// Bind the behaviour to click
		jQuery(document).on('click.exmachina.exmachina_category_checklist_toggle', '#exmachina-category-checklist-toggle', exmachina.category_checklist_toggle);
	},
	 ...
	 * the behaviour.
	 *
	 * The exmachina_toggles array is filterable in load-scripts.php before being
	...
	toggle_settings_init: function () {
		...
		jQuery.each(exmachina_toggles, function (k, v) {

			// Prepare data
			var data = {selector: v[0], show_selector: v[1], check_value: v[2]};

			// Setup toggle binding
			jQuery('div.exmachina-metaboxes').on('change.exmachina.exmachina_toggle', v[0], data, exmachina.toggle_settings);

			// Trigger the check when page loads too.
			// Can't use triggerHandler here, as that doesn't bubble the event up to div.exmachina-metaboxes.
			...
			jQuery(v[0]).trigger('change.exmachina_toggle', data);
		});
	...
	/**
	 * Have all form fields in ExMachina metaboxes set a dirty flag when changed.
	 *
	 * @since 2.0.0
	 *
	 * @function
	...
	attachUnsavedChangesListener: function () {
		...
		jQuery('div.exmachina-metaboxes :input').change(function(){
			exmachina.registerChange();
		});
		window.onbeforeunload = function(){
			if ( exmachina.settingsChanged )
				return exmachinaL10n['saveAlert'];
		};
		jQuery('div.exmachina-metaboxes input[type="submit"]').click(function(){
	...
	registerChange: function () {
		...
		exmachina.settingsChanged = true;
	},

	/**
	 * Ask user to confirm that a new version of ExMachina should now be installed.
	 *
	 * @since 2.0.0
	 *
	 * @function
	...
	confirmUpgrade: function () {
		...
		return confirm(exmachinaL10n['confirmUpgrade']);
	},
	...
	confirmReset: function () {
		...
		return confirm(exmachinaL10n['confirmReset']);
	},
	...
	ready: function () {
		...
		exmachina.category_checklist_toggle_init();

		// Initialise settings that can toggle the display of other settings
		exmachina.toggle_settings_init();

		// Initialise form field changing flag.
		exmachina.attachUnsavedChangesListener();

		// Bind character counters
		jQuery('#exmachina_title, #exmachina_description').on('keyup.exmachina.exmachina_character_count', exmachina.update_character_count);

		// Bind layout highlighter behaviour
		jQuery('.exmachina-layout-selector').on('change.exmachina.exmachina_layout_selector', 'input[type="radio"]', exmachina.layout_highlighter);

		// Bind upgrade confirmation
		jQuery('.exmachina-js-confirm-upgrade').on('click.exmachina.exmachina_confirm_upgrade', exmachina.confirmUpgrade);

		// Bind reset confirmation
		jQuery('.exmachina-js-confirm-reset').on('click.exmachina.exmachina_confirm_reset', exmachina.confirmReset);

	}

};

jQuery(exmachina.ready);

/**
 * Helper function for confirming a user action.
 *
 * This function is deprecated in favour of exmachina.confirm(text) which provides
 * the same functionality.
 *
 * @since 1.0.0
 * @deprecated 1.8.0
 */
function exmachina_confirm(text) {
	'use strict';
	return exmachina.confirm(text);
}
