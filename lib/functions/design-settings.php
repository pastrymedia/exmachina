 ...
 * @since 1.0.0
 */
function exmachina_get_fresh_design_option( $opt ) {
	$setting = get_option( EXMACHINA_DESIGN_SETTINGS_FIELD );
 ...
 * @since 1.0.0
 */
function exmachina_design_css_is_minified() {
	return exmachina_get_fresh_design_option( 'minify_css' );
}
 ...
 */
function exmachina_get_admin_design_mapping() {
	// Format:
	// '#selector' => array(
	//      'property1' => 'value1',
	//      'property2' => 'value2',
	...
	$mapping = array (
		...
		'.s, #author, #email, #url, #comment, .enews #subbox, .exmachina-design-input' => array(
		...
		'input[type="button"], input[type="submit"], .exmachina-design-button' => array(
		...
		'input:hover[type="button"], input:hover[type="submit"], .exmachina-design-button:hover' => array(
	...
	return apply_filters( 'exmachina_get_admin_design_mapping', $mapping );
}
 ...
 */
function exmachina_create_design_options( $compare, $type ) {
	...
	switch( $type ) {
		...
		case "family":
			...
			$options = apply_filters( 'exmachina_design_font_family_options', $options );
			sort( $options );
	...
	if ( is_array($options) ) {
		...
		foreach ($options as $option) {
			$output .= '<option value="'. esc_attr($option[1]) . '" title="' . esc_attr($option[1]) . '" ' . selected(esc_attr($option[1]), esc_attr($compare), false) . '>' . __($option[0], 'exmachina' ) . '</option>';
		}
	} else {
		$output = '<option>'.__( 'Select type was not valid.', 'exmachina' ).'</option>';
	}
	return $output;
}
 ...
 * You can also use no function, and just output a translatable string in exmachina_admin_setting_line().
 */
 ...
 * @param string $type One of the types allowed in {@link exmachina_create_design_options()}
 * @since 1.0.0
 * @return string HTML markup
 */
function exmachina_design_add_select_setting( $id, $label, $type ) {
	return exmachina_add_admin_label( $id, $label ) . '<select id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" class="' . $type . '-option-types">' . exmachina_create_design_options( exmachina_get_fresh_design_option( $id ), $type ) . '</select>';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_color_setting( $id, $label ) {
	return exmachina_add_admin_label( $id, $label ) . '<input type="text" id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" size="8" maxsize="7" value="' . esc_attr( exmachina_get_fresh_design_option( $id ) ) . '" class="color-picker" />';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_background_color_setting( $id, $label ) {
	return exmachina_design_add_select_setting( $id.'_select', $label, 'background' ) . '<input type="text" id="' . $id . '_hex" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" size="8" maxsize="7" value="' . esc_attr( exmachina_get_fresh_design_option( $id ) ) . '" class="color-picker" />';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_size_setting( $id, $label, $size = 1 ) {
	return exmachina_add_admin_label( $id, $label, false ) . '<input class="numeric" type="text" id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" size="' . $size . '" value="' . esc_attr( exmachina_get_fresh_design_option( $id ) ) . '" /><abbr title="pixels">px</abbr></label>';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_em_size_setting( $id, $label, $size = 3 ) {
	return exmachina_add_admin_label( $id, $label, false ) . '<input class="numeric" type="text" id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" size="' . $size . '" value="' . esc_attr( exmachina_get_fresh_design_option( $id ) ) . '" /></label>';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_text_setting( $id, $label, $size = 25 ) {
	return exmachina_add_admin_label( $id, $label ) . '<input type="text" id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" size="' . $size . '" value="' . esc_attr( exmachina_get_fresh_design_option( $id ) ) . '" />';
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_checkbox_setting( $id, $label ) {
	return '<input type="checkbox" id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" value="true" class="checkbox" ' . checked(exmachina_get_fresh_design_option( $id ), 'true', false) . '/> ' . exmachina_add_admin_label( $id, $label, true, false );
}
 ...
 * @return string HTML markup
 */
function exmachina_design_add_textarea_setting( $id, $label, $cols = 25, $rows = 10 ) {
	return exmachina_add_admin_label( $id, $label ) . '<br /><textarea id="' . $id . '" name="' . EXMACHINA_DESIGN_SETTINGS_FIELD . '[' . $id . ']" cols="39" rows="10">' . exmachina_get_fresh_design_option( $id ) . '</textarea>';
}
 ...
 * @since 1.0.0
 */
function exmachina_design_add_border_setting( $id, $label ) {
	return array( exmachina_design_add_size_setting( $id, $label ), exmachina_design_add_color_setting($id .'_color', '' ), exmachina_design_add_select_setting( $id . '_style', '', 'border' ) );
}
 ...
 * @since 1.0.0
 */
function exmachina_design_add_note( $note ) {
	return '<span class="description"><strong>' . __( 'Note', 'exmachina' ) . ':</strong> ' . $note . '</span>';
}
 ...
 * @since 1.0.0
 */
function exmachina_admin_setting_line( $args ) {
	if ( is_array( $args ) ) {
		$output = '';
		foreach ( $args as $arg ) {
			$output .= ' ' .$arg;
		}
		exmachina_admin_setting_line( $output );
	} else { ?>
			<p><?php echo $args; ?></p>
	<?php
	}
 ...
 */
function exmachina_add_admin_label( $id, $label, $add_end_tag = true, $add_colon = true ) {
	$return = '';
	if ( strlen( $label ) > 0) {
