...
/**
 * Register ExMachina default layouts.
 *
 * ExMachina comes with six layouts registered by default. These are:
 *
...
/**
 * Register new layouts in ExMachina.
 *
 * Modifies the global `$_exmachina_layouts` variable.
 *
 ...
 * `exmachina_set_default_layout()` function to ensure only one layout is set as the default at one time.
 *
 * @since 1.4.0
 *
 * @see exmachina_set_default_layout() Set a default layout.
 *
 * @uses EXMACHINA_ADMIN_IMAGE_URL URL path to admin images.
 *
...
/**
 * Unregister a layout in ExMachina.
 *
 * Modifies the global $_exmachina_layouts variable.
 *
 * @since 1.4.0
 ...
 *
 * You must manually wrap this in an HTML element with the class of `exmachina-layout-selector` in order for the CSS and
 ...
 *
 * The ExMachina admin script is enqueued to ensure the layout selector behaviour (amending label class to add border on
 ...
 *
 * A check is made to see if the `$context` is in the `exmachina-structural-wraps` theme support data. If so, then the
